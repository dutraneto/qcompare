  ...
  return (
    ...
    <div className="w-full z-50 flex justify-between">
      <div className="flex flex-col justify-center">
        <p className="text-black dark:text-blue">Your location is <strong>{name} - {country}</strong>.</p>
        <p className="text-black dark:text-blue">It‘s <strong>{tempC} ºC</strong> over there!</p>
      </div>
      <div className="flex flex-col items-center">
        ...
        />
       <small className="text-slate-900 dark:text-blue">{weatherIdx?.description}</small>
      </div>
    </div>
  </div>
  )
...
function Weather() {
  ...
  const weatherResponse = useApi(`${weatherApiUrl}?lat=${lat}&lon=${lon}&appid=${process.env.NEXT_PUBLIC_WEATHER_API}`)
